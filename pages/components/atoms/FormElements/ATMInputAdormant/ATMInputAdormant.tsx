...
const ATMInputAdormant = ({
  ...
  return (
    <div className="">
      ...
      <div
        className={twMerge(
          `${getHeight(size)} w-full border border-slate-400 rounded ${
            label && "mt-1"
          } flex items-center ${isFocus && "border-blue-400"} ${className} `
        )}
      >
        ...
        <input
          ...
          className={`w-full h-full p-1 text-slate-700 border-0 outline-0 rounded ${inputProps?.className} `}
          disabled={disabled}
          readOnly={readonly}
          onFocus={() => {
            setIsFocus(true);
