    ...
    <div className="relative">
      ...
      >
        <label
          className={`absolute left-2 transition-all duration-200  ${
            focused || value
              ...
              : "top-1/2 transform -translate-y-1/2 text-xs text-slate-400 cursor-text"
          }  ${!isOutlined && "hidden"} `}
        >
          {label}
        </label>
        <input
          ...
          className={`rounded-md w-full bg-inherit focus:outline-none px-2 py-1 ${className} placeholder:text-md text-md ${
            !isOutlined && "h-full"
          }`}
          disabled={disabled}
